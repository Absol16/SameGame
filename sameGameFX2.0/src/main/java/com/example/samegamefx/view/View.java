package com.example.samegamefx.view;
...
public class View {
    ...
    public void paintCase() {
        ...
        for (int i = 0; i < facade.getBoard().getHeight(); i++) {
            for (int j = 0; j < facade.getBoard().getWidth(); j++) {
                if (facade.getTableau()[i][j].getColor() == ColorEnum.NONE){
    ...
    public Difficulty askingLevelColor() {
        System.out.println("————————————————————————————————————————");
        System.out.println();
        ...
        return text.equals("easy") ? Difficulty.EASY: text.equals("medium") ? Difficulty.MEDIUM : Difficulty.HARD;
    }
